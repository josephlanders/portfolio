 ...
 * 
 * Joins 2 data files using the Nested Loop method.
 ****************************************************************************/
...
void simple_join(DataStore *d, int fp, int fp2)
{
    /*
    ...
    */
    if (d -> outerRecordType == guild)
    {
        while (1)
        {
            if (read_guild_records_to_outer_buffer_solaris(d, fp) == FALSE)
            {
                break;
            }

            search_guild_buffer_for_character_solaris(d, fp2);
    ...
    else
    {
        ...
        while (1)
        {
            if (read_character_records_to_outer_buffer_solaris(d, fp) == FALSE)
            {
                break;
            }

            search_character_buffer_for_guild_solaris(d, fp2);
 ...
 */
void process_files(DataStore *d, int fp, int fp2)
{
...
int main(int argc, char * argv[])
{
    ...
    fp = open_file_for_read_solaris(&d, argv[2 + offset]);
    fp2 = open_file_for_read_solaris(&d, argv[3 + offset]);

    /* Do all the joining here */
